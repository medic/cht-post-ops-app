...
describe('Scheduled messages task', () => {
    ...
    it('no contact, don\'t show the task', async () => {
        const tasks = await harness.getTasks({ title: 'task.scheduled-msgs.title' });
    ...
    it('show the task until day 365', async () => {
        harness.state.contacts.push(patient);
        let tasks = await harness.getTasks({ title: 'task.scheduled-msgs.title' });
        expect(tasks).to.have.property('length', 1);
        harness.flush(365);
        tasks = await harness.getTasks({ title: 'task.scheduled-msgs.title' });
        expect(tasks).to.have.property('length', 1);
    });
